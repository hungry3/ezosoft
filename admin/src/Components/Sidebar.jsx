...
<div className='mt-[13px] max-w-[238px] w-full border border-[#A3A3A3] '></div>
     ...
     {!isCollapsed &&(<h2 className=' font-[Poppins] text-[20px] leading-[30px] font-semibold text-white hidden sm:hidden md:block'>Dashboard</h2>)}
      ...
      <NavLink to='/user' className={`mt-[37px] flex  gap-[20px] items-center cursor-pointer ${activeTab==='user' ? 'bg-lightBlue text-black py-[5px] px-[10px] rounded-md' :''} `}>
     <img src={user} alt='dashboard' className={`w-[20px] h-[20px] ${activeTab==='user'?'active_tab':''}`}  />
     {!isCollapsed &&(<h2 className={` font-[Poppins] text-[20px] leading-[30px] font-semibold hidden sm:hidden md:block  ${activeTab === 'user' ? 'text-black' :'text-white'} ` }>User</h2>)}
   ...
   {/* pricing */}
    ...
    <NavLink to='/pricing' className={`mt-[37px] flex items-center gap-[20px] cursor-pointer ${activeTab==="pricing"? 'bg-lightBlue text-black py-[5px] px-[10px] rounded-md':''}`}  onClick={() =>handleActiveTab('pricing')}>

     <img src={Pricing} alt='pricing' className='w-[20px] h-[20px]'/>
     
     {!isCollapsed &&(<h2 className={`font-[Poppins] text-[20px] leading-[30px] font-semibold  hidden sm:hidden md:block ${activeTab ==="pricing" ? ' text-black' :' text-white'}`}>Pricing</h2>)}
